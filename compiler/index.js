...
export async function Compile(options) {
    ...
    try { await fs.promises.rm(path.join(RootPath, "assets/modules"), { recursive: true, force: true }); /* Delete the modules folder from assets */ } catch (error) { }
    ...
    await bundle.write({
        ...
        format: 'es',
        minify: !options.dev, // minify in production mode
        sourcemap: options.dev, // include sourcemaps in development mode
        chunkFileNames : chunkFileNames
    });

    await bundle.close();
    await styles.write(stylesheetPath,RootPath, options.dev);
}
