...
export async function ModifyCSS(css,scope){
    let plugins = [nested(), nestedProps(), compressVariables(), combineDuplicated(), autoprefixer(), cssnano()];
...
export async function WriteStyleSheet(document,scope,options){
    ...
    document.querySelectorAll('nijor-style').forEach(async styleTag=>{
        ...
        if(theme==="normal"){
            ...
            await appendFile(options.stylesheetPath,await ModifyCSS(`body[theme="${theme}"]{${CSS}}`));
        }
    });
}